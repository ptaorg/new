...
入口ページは、利用者が「自分の立場」「最初の確認」「使う文書」を短時間で判断できる構成にします。詳しい法的整理、調査報告、一次資料は深いページに置き、入口ページでは結論・確認事項・次の行動を優先します。
...
- `reality.html` は、実物資料を含む `compliance.html` へ統合し、旧URLは静的リダイレクトとして残す。
